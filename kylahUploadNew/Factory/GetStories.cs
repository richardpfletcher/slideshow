...
namespace Stories.Factory
{
    public class GetStories
    {
        ...
        public int UpdateDish(int id, string title, string url, string picture)
        {
            var p = new DynamicParameters();
            ...
            {
                sqlConnection.Open();
                const string storedProcedure = "dbo.UpdateDish";
                var values = sqlConnection.Query<ReceipeTotalModel>(storedProcedure, p, commandType: CommandType.StoredProcedure);
                //foreach (var el in values)
                //{
                //    total = el.totalReceipesInt;
        ...
        public int UpdateCoupon(int ID, string titleCoupon, string urlCoupon, string pictureCoupon, string startDate, string endDate, string dayOfWeek, string dishID)
        {
            var p = new DynamicParameters();
            ...
            p.Add("@ID", ID);
            p.Add("@titleCoupon", titleCoupon);
            p.Add("@urlCoupon", urlCoupon);
            p.Add("@pictureCoupon", pictureCoupon);

            p.Add("@startDate", startDate);
            p.Add("@endDate", endDate);
            p.Add("@dayOfWeek", dayOfWeek);
            p.Add("@dishID", dishID);
            ...
            {
                sqlConnection.Open();
                const string storedProcedure = "dbo.UpdateCoupon1";
                var values = sqlConnection.Query<ReceipeTotalModel>(storedProcedure, p, commandType: CommandType.StoredProcedure);
                //foreach (var el in values)
                //{
                //    total = el.totalReceipesInt;
